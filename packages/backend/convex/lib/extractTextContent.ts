...
const AI_MODELS = {
    ...
    html: google("gemini-1.5-flash"), // same model handles all 3
}
...
export async function extractTextContent(
    ...
    if(mimeType.toLowerCase().includes("pdf")){
        return extracPdfText(url, mimeType, filename)
    }

    if(mimeType.toLowerCase().includes("text")){
        return extracTextFileContent(ctx, storageId, bytes, mimeType)
    }
...
async function  extracTextFileContent(
    ctx: {storage: StorageActionWriter},
    storageId: Id<"_storage">,
